...
public class Transition {
    ...
    public static void to(BaseScreen newScreen, EffectType type, boolean immediate) {
    ...
    public static void render(SpriteBatch batch) {
        ...
        next.render(Time.delta);
        fbo.transition.end();

        // render current screen to a buffer
        fbo.original.begin();
        Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT);
        Main.game.currentScreen.render(Time.delta);
        fbo.original.end();
